			...
			series: [
				{
					...
					tooltip: {
						...
						pointFormatter: function () {
							...
							return `Data: ${date}<br/>` + `Elevação: nível ${elevationLevel}: <b>${customLabels[elevationLevel]}</b><br/>` + `Velocidade: <b>${speed} m/s</b><br/>` + `Direção: <b>${direction}°</b>`
						},
					},
				},
			],
			...
			series: [
				{
					...
					tooltip: {
						...
						headerFormat: "Direção e velocidade do vento com monóxido de carbono<br/>",
						pointFormatter: function () {
							const date = Highcharts.dateFormat("%d/%m %H:%M", this.x)
							const elevationLevel = this.y

							// Encontra os dados na série heatmap
							const heatmapIndex = dataCo.findIndex((item) => item[0] === this.x && item[1] === this.y)
							const co = heatmapIndex !== -1 ? dataCo[heatmapIndex][2] : "não disponível"

							// Encontra a velocidade e direção do vento na série vector (dataWind)
							const windPoint = dataWind.find((item) => item[0] === this.x && item[1] === this.y)
							const speed = windPoint ? windPoint[2] : "não disponível"
							const direction = windPoint ? windPoint[3] : "não disponível"

							return `Data: ${date}<br/>` + `Elevação: nível ${elevationLevel}: <b>${customLabels[elevationLevel]}</b><br/>` + `Velocidade: <b>${speed} m/s</b><br/>` + `Direção: <b>${direction}°</b><br/>` + `Concentração de CO: <b>${co} ppm</b>`
						},
					},
					point: {
						events: {
							mouseOver: function () {
								...
								const correspondingWind = vectorData.data.find((point) => point.x === this.x && point.y === this.y)
			...
			series: [
				{
					...
					tooltip: {
						shared: true, // Compartilha o tooltip com heatmap e vetor
						headerFormat: "Direção e velocidade do vento com material micro-particulado 2.5nm<br/>",
						pointFormatter: function () {
							const date = Highcharts.dateFormat("%d/%m %H:%M", this.x)
							const elevationLevel = this.y

							// Encontra os dados na série heatmap
							const heatmapIndex = dataPm25.findIndex((item) => item[0] === this.x && item[1] === this.y)
							const pm25 = heatmapIndex !== -1 ? dataPm25[heatmapIndex][2] : "não disponível"
							...
							return `Data: ${date}<br/>` + `Elevação: nível ${elevationLevel}: <b>${customLabels[elevationLevel]}</b><br/>` + `Velocidade: <b>${speed} m/s</b><br/>` + `Direção: <b>${direction}°</b><br/>` + `Material Micro-particulado 2.5nm: <b>${pm25} ug/m³</b>`
						},
					},
					point: {
						events: {
			...
			series: [
				{
					...
					tooltip: {
						shared: true, // Compartilha o tooltip com heatmap e vetor
						headerFormat: "Direção e velocidade do vento com óxido de nitrogênio<br/>",
						pointFormatter: function () {
							const date = Highcharts.dateFormat("%d/%m %H:%M", this.x)
							const elevationLevel = this.y

							// Encontra os dados na série heatmap
							const heatmapIndex = dataNox.findIndex((item) => item[0] === this.x && item[1] === this.y)
							const nox = heatmapIndex !== -1 ? dataNox[heatmapIndex][2] : "não disponível"
							...
							return `Data: ${date}<br/>` + `Elevação: nível ${elevationLevel}: <b>${customLabels[elevationLevel]}</b><br/>` + `Velocidade: <b>${speed} m/s</b><br/>` + `Direção: <b>${direction}°</b><br/>` + `Óxido de nitrogénio (NOx): <b>${nox} ug/m³</b>`
						},
					},
					point: {
						events: {
